...
@item_router.post("/")
def create_item(request: Request, item: Item, item_repo: ItemRepository = Depends(lambda: injector.get(ItemRepository))):
    item_repo.new(item, request.state.user['login'])
    return {"message": "Item created successfully"}

@item_router.post("/batch")
def create_items(request: Request, items: List[Item], item_repo: ItemRepository = Depends(lambda: injector.get(ItemRepository))):
    try:
...
def update_item(request: Request, item_id: int, item: Item, item_repo: ItemRepository = Depends(lambda: injector.get(ItemRepository))):
